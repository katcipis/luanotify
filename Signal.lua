...
local has_function = get_function_position   

local function has_handler_table(handlers, handler_function)
          pos, handler_table = get_handler_table(handlers, handler_function)
...
function Signal:connect(handler_function)
    if(not has_handler_table(self._handlers, handler_function)) then
...
function Signal:add_set_up(set_up_func)
    if(not has_function(self._set_up_funcs, set_up_func)) then
...
function Signal:remove_set_up(set_up_func)
    pos = get_function_position(self._set_up_funcs, set_up_func)
    if(pos) then table.remove(self._set_up_funcs, pos) end
end


function Signal:add_tear_down(tear_down_func)
    if(not has_function(self._tear_down_funcs, tear_down_func)) then
...
function Signal:remove_tear_down(tear_down_func)
    pos = get_function_position(self._tear_down_funcs, tear_down_func)
